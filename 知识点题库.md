...
- [ ] 从 GitHub 开源项目提取知识点标签体系
  ...
  - [ ] 整理语言领域知识点（将现有 17 个科目升级为带知识点的领域）：
...
- [ ] 新增知识点查询接口
  ...
  - [ ] 读取 `knowledge_points.json`，返回知识点分类树
  - [ ] 支持按领域类型筛选（`?type=general` 或 `?type=language`）
  - [ ] 返回格式：领域列表 → 每个领域下的知识点列表（含 id、名称、难度等级）
- [ ] 改造出题 API 端点
  - [ ] 修改 `POST /api/quiz/generate`，将参数从 `topic + question_type` 改为 `domain_id + topic_id + question_type + difficulty`
    - `domain_id`：领域 ID（如 `dsa`、`python`、`javascript`）
    - `topic_id`：知识点 ID（如 `dp`、`py_decorator`）
    - `question_type`：题型（保留现有四种）
    - `difficulty`：难度（简单/中等/困难）
  - [ ] 从 `knowledge_points.json` 加载对应知识点的元数据（description、typical_approach）
  - [ ] 使用新的知识点出题提示词构建 AI 请求
  - [ ] 记忆上下文（`_build_memory_context`）改为按 `domain_id + topic_id` 查询历史答题记录
  - [ ] `QuizRecord` 保存时，`topic` 字段存领域名称，`knowledge_point` 字段存知识点名称
- [ ] 新增知识点出题提示词
  - [ ] 在 `backend/app/prompts/quiz.py` 中新增 `get_knowledge_quiz_prompt()` 函数
  - [ ] 提示词包含：领域名称、知识点名称、知识点描述、典型解题思路、难度要求、题型要求
  - [ ] 强调原创生成，不得搬运已有题目
  - [ ] 支持中英文双语
  - [ ] 可删除或保留旧的 `get_quiz_generate_prompt()`（保留则作为向后兼容的备用）
- [ ] 新增举一反三提示词与接口
  - [ ] 在 `backend/app/prompts/quiz.py` 中新增 `get_variant_quiz_prompt()` 函数
  - [ ] 接收上一题的 question、knowledge_point、user_answer、is_correct 信息
  - [ ] 提示词指导 AI 生成同一知识点但变换了场景/数据/条件的变体题
  - [ ] 答错时变体题难度持平，答对时难度可略有提升
...
- [ ] 新增知识点掌握度统计接口
  - [ ] 新增 `GET /api/quiz/knowledge-stats` 端点
  - [ ] 按知识点维度统计：每个知识点的答题数、正确数、正确率、掌握状态
  - [ ] 按领域维度聚合：每个领域的整体正确率、已掌握/薄弱/未开始的知识点数
  - [ ] 掌握状态判定：复用现有的 `MASTERY_THRESHOLD=2` 逻辑
  - [ ] 现有的 `GET /api/quiz/stats` 保留，返回全局统计（总题数、正确率、掌握/薄弱知识点）

### Phase 3: 前端刷题页面重构

- [ ] 重构 `QuizPage.jsx` 顶部区域：领域选择
  - [ ] 移除现有的 17 个科目选择 Tag 组件
  - [ ] 替换为领域选择组件，分两组展示：
  ...
  - [ ] 选中领域后进入知识点选择
- [ ] 新增知识点选择面板
  - [ ] 选中领域后，下方展示该领域下的知识点列表
  - [ ] 每个知识点卡片/Tag 显示：名称 + 掌握进度（小型进度条或颜色标识：绿色=已掌握、橙色=薄弱、灰色=未开始）
  - [ ] 选中知识点后显示：
    - 知识点描述（1-2 句话）
    - 典型解题思路
    - 难度选择（简单/中等/困难，默认中等）
  ...
  - [ ] 点击"开始出题"生成第一题
- [ ] 新增前端 API 调用
  - [ ] 在 `frontend/src/api/quiz.js` 中：
    - 新增 `getKnowledgePoints(type?)` — 获取知识点分类树
    - 新增 `generateVariant(data)` — 生成变体题
    - 新增 `getKnowledgeStats()` — 获取知识点维度统计
    - 修改 `generateQuiz()` 参数为 `(domainId, topicId, questionType, difficulty)`
- [ ] 实现举一反三交互
  - [ ] 用户答错或跳过后，在结果区域（标准答案和解析下方）显示"举一反三"按钮
  - [ ] 点击后调用 `generateVariant()` 获取同知识点变体题
  - [ ] 变体题带有"变体练习"标签，与普通题区分
  - [ ] 答对时也可选择"继续深入"生成更高难度的变体
- [ ] 知识点掌握度可视化
  - [ ] 重构右侧统计面板：
